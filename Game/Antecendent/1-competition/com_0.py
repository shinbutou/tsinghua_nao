...
def click(args):
    ...
    if button_num == 1:
        if card_n1 < card_n2:
            ...
            # tts.say(str(result_match[0]) + str('，你竟然选对了'))
            ...
            # tts.say(str(result_mismatch[0]) + str('，你选错了'))
            # leds.fade('FaceLeds', 0.5, 1)
...
count_lose_l = tk.Label(frame, text='失败次数:   %d' % count_lose, bg='lightblue', font=("微软雅黑", 16), width=tmp, fg='black',
                        height=4)
count_grades_l = tk.Label(frame, text='当前成绩：Nao   %d    我   %d' % (count_grades_nao, count_grades_self), bg='lightblue', font=("微软雅黑", 16), width=tmp,
                          fg='black', height=4)
