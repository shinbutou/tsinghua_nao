...
# 新建窗口，设定大小
deck = tk.Tk()
deck.title("Play with Nao Robot")
...
cardback = ImageTk.PhotoImage(Image.open("images_0/robot_cooperation.png"))
cards = []
names = []
count_match = 0
count_lose = 0
count_grades = 10 * (count_match - count_lose)
...
def rank_cards():
    ...
    label_1 = tk.Label(frame_top, bg='lightblue', text='BONUS TIME:  请根据刚刚的互动过程猜测 Nao 对这些地点的喜爱度并按顺序点击', font=('微软雅黑', 24), fg='white', width=deck.winfo_screenwidth(), height=4)
    label_1.pack()
    ...
    # 定义好9个button
    b_1 = tk.Button(frame_r, image=cards[random_list[0]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(1, random_list[0], str(names[session][random_list[0]])): rank_record(j))
    b_2 = tk.Button(frame_r, image=cards[random_list[1]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(2, random_list[1], names[session][random_list[1]]): rank_record(j))
    b_3 = tk.Button(frame_r, image=cards[random_list[2]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(3, random_list[2], names[session][random_list[2]]): rank_record(j))
    b_4 = tk.Button(frame_r, image=cards[random_list[3]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(4, random_list[3], names[session][random_list[3]]): rank_record(j))
    b_5 = tk.Button(frame_r, image=cards[random_list[4]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(5, random_list[4], names[session][random_list[4]]): rank_record(j))
    b_6 = tk.Button(frame_r, image=cards[random_list[5]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(6, random_list[5], names[session][random_list[5]]): rank_record(j))
    b_7 = tk.Button(frame_r, image=cards[random_list[6]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(7, random_list[6], names[session][random_list[6]]): rank_record(j))
    b_8 = tk.Button(frame_r, image=cards[random_list[7]], width=250, height=250, borderwidth=1, padx=10, pady=10,
                    command=lambda j=(8, random_list[7], names[session][random_list[7]]): rank_record(j))
    b_9 = tk.Button(frame_r, image=cards[random_list[8]], width=250, height=250, borderwidth=1, padx=10, pady=10,
    ...
    def rank_record(args):
        ...
        if rank_ID == 9:
            ...
            csv_name = time.time()
            dataframe.to_csv('%s.csv' % str(csv_name), index=False, sep=',')
            # tkMessageBox.showinfo('end', '本次任务已结束，请告知实验人员\n\n十分感谢！', parent=deck)


# 定义函数refresh
def refresh():
    global random_list, session, period, count_match, count_lose, names, count_grades
    print (period)
    random.shuffle(random_list)
...
# 定义函数click
def click(args):
    global names, random_list, period, session, count_match, count_lose, cardback, count_grades
    period += 1
    button_num = args[0]
    card_n1 = args[1]
    card_n2 = args[2]
...
topic = ['西湖风光', '民族风情', '城市建筑', '四季风景', '剪影星空']

name_1 = ['断桥残雪', '花港观鱼', '雷峰夕照', '柳浪闻莺', '平湖秋月', '曲院风荷', '三潭映月', '双峰插云', '苏堤春晓']
name_2 = ['灯笼', '飞燕', '牛牛', '古楼', '荷花灯', '火车', '金鱼', '舞狮', '燕子']
name_3 = ['深圳', '上海', '青岛', '广州', '北京',  '成都', '苏州', '西安', '重庆']
name_4 = ['春分', '大寒', '白露', '大雪', '寒露', '立冬', '立秋', '立夏', '小暑']
name_5 = ['彩色星空', '湖边星空', '黄昏星空', '蓝绿星空', '魅影星空', '山脉星空', '沙漠星空', '峰峦插云', '流云星空']

names = [name_1, name_2, name_3, name_4, name_5]

tmp = deck.winfo_screenwidth() / 70
session_l = tk.Label(frame, text='当前主题：%s' % str(topic[session]), bg='lightblue', font=("微软雅黑", 16), fg='black',
                     width=tmp, height=4)
period_l = tk.Label(frame, text='第%d轮' % (period + 1), bg='lightblue', font=("微软雅黑", 16), fg='black', width=tmp,
                    height=4)
...
label_r = tk.Label(frame_l, bg='white', font=("微软雅黑", 14), fg='black', text='希望我们能猜对！', width=25, height=7)
label_r.pack()
