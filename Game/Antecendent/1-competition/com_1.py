...
# 新建窗口，设定大小
deck = tk.Toplevel()
deck.title("Play with Nao Robot - Round 2")
...
grades = pd.read_csv('competition1.csv')
...
def rank_cards():
    ...
    def rank_record(args):
        ...
        if rank_ID == 9:
            ...
            dataframe.to_csv('competition2.csv', index=False, sep=',')
...
def refresh():
    ...
    if period == 21:
        ...
        dataframe_choice.to_csv('competition2_choice.csv', index=False, sep=',')
        rank_cards()
    else:
        del cards[:]
        for x in range(1, 10):
...
tts.say('你不会都猜对了吧？下一组图片，我会继续优化展示策略！')
