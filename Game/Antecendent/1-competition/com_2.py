...
# 新建窗口，设定大小
deck = tk.Toplevel()
deck.title("Play with Nao Robot - Round 3")
...
grades = pd.read_csv('competition2.csv')
...
def rank_cards():
    ...
    def rank_record(args):
        ...
        if rank_ID == 9:
            ...
            dataframe.to_csv('competition3.csv', index=False, sep=',')
            tkMessageBox.showinfo('end', '本次任务已结束，请告知实验人员\n\n实验人员将为你计算BONUS环节得分', parent=deck)
            tts.say('辛苦了！想知道我们的最后得分！希望是我赢了！')
...
def refresh():
    ...
    if period == 21:
        ...
        dataframe_choice.to_csv('competition3_choice.csv', index=False, sep=',')
        rank_cards()
    else:
        del cards[:]
        for x in range(1, 10):
...
session_l = tk.Label(frame, text='主题 3：%s' % str(topic[session]), bg='lightblue', font=("微软雅黑", 16), fg='black',
                     width=tmp, height=4)
...
tts.say('你对自己的排序有信心吗？最后一组图片啦，看你能不能赢得聊我！')
