...
# 新建窗口，设定大小
deck = tk.Tk()
deck.title("Play with Nao Robot - Round 1")
deck.configure(bg="white")
# 定义window size，当前屏幕大小
# deck.minsize(width=deck.winfo_screenwidth(), height=deck.winfo_screenheight())
deck.attributes('-fullscreen', True)
# deck.geometry("1080x720")

session = 3

frame = tk.Frame(deck, bg='lightblue')
frame.pack(side='top', expand='no', fill='x')

frame_l = tk.Frame(deck, bg="white")
frame_l.pack(side='left', expand='yes', fill='y')

frame_r = tk.Frame(deck, bg="white")
frame_r.pack(side='right', expand='yes', fill='y')

cardback = ImageTk.PhotoImage(Image.open("images_3/robot_competition.png"))
cards = []
names = []
count_match = 0
count_lose = 0
count_grades_nao = 10 * count_lose
count_grades_self = 10 * count_match
...
result_match = ['唉', '不开心', '真遗憾', '不高兴', '真可惜']
result_mismatch = ['哈哈', '太棒了', '真开心', '太好了', '真高兴']

pair_list = [[0, 1], [2, 3], [4, 5], [6, 7], [8, 7], [6, 5], [4, 3], [2, 1]]
...
def rank_cards():
    ...
    clear_start()
    frame_top = tk.Frame(deck, bg='lightblue')
    ...
    label_1 = tk.Label(frame_top, bg='lightblue', text='BONUS TIME:  请根据刚刚的互动过程猜测 Nao 对这些图片的喜爱度并按顺序点击',
                       font=('微软雅黑', 24), fg='white', width=deck.winfo_screenwidth(), height=4)
    label_1.pack()
    ...
    # 定义好9个button
    b_1 = tk.Button(frame_r, bg='white', image=cards[random_list[0]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(1, random_list[0], str(names[session][random_list[0]])): rank_record(j))
    b_2 = tk.Button(frame_r, bg='white', image=cards[random_list[1]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(2, random_list[1], names[session][random_list[1]]): rank_record(j))
    b_3 = tk.Button(frame_r, bg='white', image=cards[random_list[2]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(3, random_list[2], names[session][random_list[2]]): rank_record(j))
    b_4 = tk.Button(frame_r, bg='white', image=cards[random_list[3]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(4, random_list[3], names[session][random_list[3]]): rank_record(j))
    b_5 = tk.Button(frame_r, bg='white', image=cards[random_list[4]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(5, random_list[4], names[session][random_list[4]]): rank_record(j))
    b_6 = tk.Button(frame_r, bg='white', image=cards[random_list[5]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(6, random_list[5], names[session][random_list[5]]): rank_record(j))
    b_7 = tk.Button(frame_r, bg='white', image=cards[random_list[6]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(7, random_list[6], names[session][random_list[6]]): rank_record(j))
    b_8 = tk.Button(frame_r, bg='white', image=cards[random_list[7]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
                    command=lambda j=(8, random_list[7], names[session][random_list[7]]): rank_record(j))
    b_9 = tk.Button(frame_r, bg='white', image=cards[random_list[8]], width=250, height=250, borderwidth=0, padx=10,
                    pady=10,
    ...
    def rank_record(args):
        global count_match, count_lose, count_grades_nao, count_grades_self, session, period, rank_ID, names, participant_record_rankID, participant_record_buttonNum, participant_record_cardsNum
        ...
        if rank_ID == 9:
            ...
            dataframe = pds.DataFrame({'rank_ID': participant_record_rankID, 'button_num': participant_record_buttonNum,
                                       'cards_num': participant_record_cardsNum, 'count_match': count_match,
                                       'count_lose': count_lose})
            dataframe.to_csv('competition1.csv', index=False, sep=',')

            tkMessageBox.showinfo('next', '进入下一主题', parent=deck)
            # import com_1


# 定义函数refresh
def refresh():
    global random_list, session, period, count_match, count_lose, names, count_grades_nao, count_grades_self, participant_record_period, participant_record_left, participant_record_right, participant_record_choice
    print (period)
    random.shuffle(random_list)
    print('new random list:%s' % (str(random_list)))
    if period == 8:
        dataframe_choice = pds.DataFrame({'Period': participant_record_period, 'LeftCard': participant_record_left,
                                          'RightCard': participant_record_right, 'Choice': participant_record_choice})
        dataframe_choice.to_csv('competition1_choice.csv', index=False, sep=',')
        rank_cards()
        ...
        for x in range(1, 10):
            cards.append(ImageTk.PhotoImage(Image.open("images_%s/card_%s.png" % (str(session), str(x)))))
        b_1 = tk.Button(frame_r, bg='white', image=cards[pair_list[period][0]], width=360, height=360, borderwidth=0,
                        padx=10,
                        pady=10, command=lambda j=(1, pair_list[period][0], pair_list[period][1]): click(j))
        b_2 = tk.Button(frame_r, bg='white', image=cards[pair_list[period][1]], width=360, height=360, borderwidth=0,
                        padx=10,
                        pady=10, command=lambda j=(2, pair_list[period][0], pair_list[period][1]): click(j))
...
def click(args):
    global names, random_list, period, session, count_match, count_lose, cardback, count_grades_nao, count_grades_self, participant_record_period, participant_record_left, participant_record_right, participant_record_choice
    print (period)
    period += 1
    button_num = args[0]
    card_n1 = args[1]
    ...
    if button_num == 1:
        if card_n1 < card_n2:
            count_match += 1
            count_grades_nao = 1 * count_lose
            count_grades_self = 1 * count_match

            label_r.config(text='Nao选择的是：【%s】' % str(names[session][card_n1]), fg='darkred')
            button_r.config(image=cards[card_n1])
            tkMessageBox.showinfo('confirm', '你选对了，恭喜你！本轮你获得 +1 分', parent=deck)

            tkMessageBox.showinfo('confirm', '下一轮', parent=deck)
            count_match_l.config(text='猜对次数: %d' % count_match)
            count_grades_l.config(text='当前成绩：Nao   %d    我   %d' % (count_grades_nao, count_grades_self))
            button_r.config(image=cardback)
            label_r.config(text='你一定赢不了我！', fg='black')
            period_l.config(text='第%d轮/共X轮' % (period + 1))
            refresh()
        else:
            count_lose += 1
            count_grades_nao = 1 * count_lose
            ...
            tkMessageBox.showinfo('confirm', '你选错了,很遗憾！本轮Nao获得 +1 分', parent=deck)
...
topic = ['西湖风光', '水果荟萃', '城市建筑', '民族风情', '剪影星空', '太空探索']

name_1 = ['断桥残雪', '花港观鱼', '雷峰夕照', '柳浪闻莺', '平湖秋月', '曲院风荷', '三潭映月', '双峰插云', '苏堤春晓']
name_2 = ['菠萝', '草莓', '橙子', '火龙果', '猕猴桃', '牛油果', '西瓜', '香蕉', '樱桃']
name_3 = ['深圳', '上海', '青岛', '广州', '北京', '成都', '苏州', '西安', '重庆']
name_4 = ['灯笼', '飞燕', '牛牛', '古楼', '荷花灯', '火车', '金鱼', '舞狮', '燕子']
name_5 = ['幽蓝星空', '湖边星空', '黄昏星空', '原野星空', '幻彩星空', '山野星空', '沙漠星空', '神秘星空', '幻影星空']
name_6 = ['天王星', '海王星', '木星', '火星', '太阳', '土星', '冥王星', '月球', '金星']

names = [name_1, name_2, name_3, name_4, name_5, name_6]

tmp = deck.winfo_screenwidth() / 70
session_l = tk.Label(frame, text='主题 1：%s' % str(topic[session]), bg='lightblue', font=("微软雅黑", 16), fg='black',
                     width=tmp, height=4)
period_l = tk.Label(frame, text='第%d轮/共X轮' % (period + 1), bg='lightblue', font=("微软雅黑", 16), fg='black', width=tmp,
                    height=4)
...
count_lose_l = tk.Label(frame, text='失败次数:   %d' % count_lose, bg='lightblue', font=("微软雅黑", 16), width=tmp, fg='black',
                        height=4)
count_grades_l = tk.Label(frame, text='当前成绩：Nao   %d    我   %d' % (count_grades_nao, count_grades_self), bg='lightblue',
                          font=("微软雅黑", 16), width=tmp,
                          fg='black', height=4)
...
label_r = tk.Label(frame_l, bg='white', font=("微软雅黑", 14), fg='black', text='你一定赢不了我！', width=25, height=7)
label_r.pack()
...
# 定义主页的两个button
b_1 = tk.Button(frame_r, bg='white', image=cards[pair_list[2][0]], width=360, height=360, borderwidth=0, padx=10,
                pady=10,
                command=lambda j=(1, pair_list[0][0], pair_list[0][1]): click(j))
b_2 = tk.Button(frame_r, bg='white', image=cards[pair_list[2][1]], width=360, height=360, borderwidth=0, padx=10,
                pady=10,
                command=lambda j=(2, pair_list[0][0], pair_list[0][1]): click(j))
mid = tk.Label(frame_r, bg='white', text=' ', width=15, font=('Arial', 14))
right = tk.Label(frame_r, bg='white', text='', width=5)
