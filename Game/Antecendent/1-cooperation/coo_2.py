...
cards = []
names = []
grades = pd.read_csv('cooperation2.csv')
...
def rank_cards():
    ...
    label_1 = tk.Label(frame_top, bg='lightblue', text='请根据刚刚的互动过程猜测 Nao 对这些图片的喜爱度并按顺序点击',
    ...
    def rank_record(args):
        ...
        if rank_ID == 9:
            ...
            dataframe.to_csv('cooperation3.csv', index=False, sep=',')
            tkMessageBox.showinfo('end', '本次任务已结束，请告知实验人员\n\n实验人员将为你计算排序环节的得分', parent=deck)
            tts.say('你辛苦了！不知道我们有没有赢得这次任务呢？希望我们能成功！')
...
def refresh():
    ...
    if period == 21:
        ...
        dataframe_choice.to_csv('cooperation3_choice.csv', index=False, sep=',')
        rank_cards()
    else:
        del cards[:]
        for x in range(1, 10):
...
def click(args):
    ...
    if button_num == 1:
        if card_n1 < card_n2:
            ...
            tkMessageBox.showinfo('confirm', '匹配成功，恭喜你们：+1分', parent=deck)
            ...
            tkMessageBox.showinfo('confirm', '匹配失败，真遗憾：-1分', parent=deck)
        ...
        if card_n1 < card_n2:
            ...
            button_r.config(image=cards[card_n1])
            tkMessageBox.showinfo('confirm', '真遗憾：-1分', parent=deck)
...
tts.say('希望你都猜对啦！最后一组图片了，我们再接再厉！')
