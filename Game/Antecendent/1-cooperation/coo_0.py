...
count_match = 0
count_lose = 0
count_grades = count_match - count_lose
...
def rank_cards():
    ...
    def rank_record(args):
        ...
        if rank_ID == 9:
            ...
            dataframe.to_csv('cooperation1.csv', index=False, sep=',')
...
def get_grades():
    global count_match, count_lose
    return {count_match, count_lose}
...
def refresh():
    ...
    if period == 21:
        ...
        dataframe_choice.to_csv('cooperation1_choice.csv', index=False, sep=',')
        rank_cards()
    else:
        del cards[:]
        for x in range(1, 10):
